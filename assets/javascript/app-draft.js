...
questions.q1.answers[questions.q1.correctAnswerIndex]
...
function chooseQuestion () {
	// grabs math.floor/math.random question  
	// if the useCount === 0, displayQuestion (); otherwise chooseQuestion (); 
}

function dipslayQuestion () {
	chooseQuestion (); 
	count (); 
	// takes chosen question and puts it in black box on screen
	// waits 10 seconds, then moves to next question
	// if something is clicked processAnswer (); 
	// if nothing is clicked increments totalUnanswered++; and displayQuestion (); again
	// if statement that if the totalQuestionCount > 4, gameOver (); and final counts 
	// totalQuestionCount ++; 
}
...
function showAnswer () {
	...
	// if statement that if the totalQuestionCount > 4, show game over and final counts 
	gameOver (); 
}

function gameOver () {
	//shows total counts, totalCorrect and totalIncorrect and totalUnanswered, says "press start to play again"
}

function count () {
	gameState.timer--;
	var counter = setInterval(gameState.timer,1000); 
	$("#timeDisplay").html(counter);
}
...
// $(....on click...) pass it the class and then the function you want it to do... 


// should you create a function for creation display and then render it, rather than rendering all the pieces individually? 

// set same class on each answer, and have $(this)data() ? grab the answer of the button,
 // do you have to have html data or grab the text?  
// compare the data to the answer...

// how to do that with answers as true or not? vs right answer? 
